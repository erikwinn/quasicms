...
*/
	...
	abstract class QControlBase extends QBaseClass
    {
        ...
        */
        ...
        /**#@-*/
        /**
        *@var integer intOpacity - must be an integer value between 0 and 100
        */
        ...
        /**
        *@var string strValidationError - the error to be displayed if Validate fails.
        */
        protected $strValidationError = null;
        /**
        *@var boolean blnVisible - if false display is set to "none"
        */
        protected $blnVisible = true;
        /**
        *@var boolean blnEnable - if false input is disabled
        */
        protected $blnEnabled = true;
        /**
        *@var boolean blnRequired - if true and a field is empty Validate() will fail
        */
        protected $blnRequired = false;        
        /**
        *@var integer intTabIndex - indicate the order in which the TAB key focuses on this control
        */
        protected $intTabIndex = 0;
        /**
        * This member stores what type of validation to run on input. It may be an integer
        * (QCausesValidation::Foo), an array of QControls or a single QControl. Validate()
        * will be called on the control(s) indicated. See QCausesValidation for the types
        * available (Children, SiblingsAndChildren).
        * Note: passing "true" will cause the QForm to iterate through all controls calling
        * Validate for each.
        *@var mixed array | integer | QControl | boolean
        */
        ...
        /**
        * This constructor sets the parent QForm (objForm) and if it is a child of a QControl it
        * sets the parent control (objParentControl) - otherwise objParentControl is assumed
        * to be null provided the terminator for a linked list of controls.
        *
        * Note that the CSS id must contain only alphanumeric characters (not clear why ..)
        *
        *@param mixed objParentObject - must be either a QForm or QControl
        *@param string strControlId - an optional CSS id for the control
        */      
...
//				$this->objParentControl->blnModified = true;
        ...
        /**
        * This function takes the name of a class and searches in SESSION for a serialized instantiaion
        * associated with $FormId_$ControlId. If found it unserializes the object and returns it
        * - if none exists it instantiates the class and stores it in the session. It is stored in the session
        * array with an index of the form $FormId_$ControlId - FormId is obtained from either the QForm
        * passed as the second parameter or from objForm of the QControl as the second parameter.
        *
        * enote: it is unclear to me why the object is returned .. i would have Store/GetPersistant(...)
        * - i also do not understand why we would create a new class, do nothing with it and stash it
        * in SESSION .. doesn't seem very useful actually. and in fact, it is never called in the entire
        * library. 
        * it is also unclear why this is static / here, suggest a QUtilities class w/GetPersistant(...)
        * 
        *@param string strClassName - the name of the class to store
        *@param mixed objParentObject - A QControl or QForm as the parent of the class ..
        *@param string strControlId - the CSS Id of the parent control
        ...
        */
		public static function CreatePersistent($strClassName, $objParentObject, $strControlId)
        {
			...
			if (array_key_exists($objForm->FormId . '_' . $strControlId, $_SESSION)
                                        && $_SESSION[$objForm->FormId . '_' . $strControlId])
            {
        ...
        /**
        * This function prepares this control for serializaion and storage ..
        */
        ...
        /**
        * This function serializes a clone of this control and stores it in SESSION["$FormId_$ControlId"]
        * Note that ControlId is _this_ control's CSS Id.
        */
        public function Persist()
        {
            //enote: it is _entirely_ unclear to me why we clone here ..
